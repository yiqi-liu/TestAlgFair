...
for (cur in 1:length(unique(pct_vals))) { # for each unique percentile
  ...
  est_nuisance <- nuisance(X=X, G=G, Y=cur_Y, seed=cur,
                           method="grf", num.trees=5000)
...
for (iter in 1:num_rep){
  ...
  for (cur in 1:length(unique(pct_vals))) { # for each unique percentile
    cur_Y <- get(paste0("Y_", unique(pct_vals)[cur]))
    train_new_alg <- alg_on_FAfrontier(X=X, Y=cur_Y, G=G, seed=iter,
                                       method="grf", num.trees=5000,
...
write.csv(alt_alg_result, paste0('../../results/buildAlg-results/rf/alt_alg_result.csv'), row.names = F)
write.csv(alt_alg_f1a, paste0('../../results/buildAlg-results/rf/alt_alg_f1a.csv'), row.names = F)
write.csv(alt_alg_f1b, paste0('../../results/buildAlg-results/rf/alt_alg_f1b.csv'), row.names = F)
...
ggsave(paste0('../../results/buildAlg-results/rf/alt-alg-f1a.png'),
...
write.csv(table_frac_bl_trt[table_frac_bl_trt$capacity_threshold<=98, ], paste0('../../results/buildAlg-results/rf/table_frac_bl_trt.csv'), row.names = F)
