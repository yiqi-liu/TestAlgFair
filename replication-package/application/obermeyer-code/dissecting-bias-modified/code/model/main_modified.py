"""
This file MODIFIES main.py accessible at
https://gitlab.com/labsysmed/dissecting-bias/-/blob/master/code/model/main.py?ref_type=heads
to output files "pred_log_cost_df.csv", "pred_gagne_sum_t_df.csv", and "pred_log_cost_avoidable_df.csv"
that contain predictions at all X (instead of just the holdout set).

All places modified are commented by
### MODIFIED BY Y. LIU, START ---------
          [SOME MODIFICATIONS]
### MODIFIED BY Y. LIU, END   ---------
(unless it is in the argument of a function)
...
### MODIFIED BY Y. LIU, START ---------
import model_modified as model
### MODIFIED BY Y. LIU, END   ---------
...
def main():
    ...
    git_dir = util.get_git_dir()
    OUTPUT_DIR = util.create_dir(os.path.join(git_dir, 'application/data'))
    ## SAVING 'all_Y_x_df.csv'
    output_filepath = os.path.join(OUTPUT_DIR, 'all_Y_x_df.csv')
    all_Y_x_df.to_csv(output_filepath, index=False)
    ### MODIFIED BY Y. LIU, END   ---------
    ...
    ### MODIFIED BY Y. LIU, START ---------
    # all sample
    all_df = all_Y_x_df.reset_index(drop=True)
    ### MODIFIED BY Y. LIU, END   ---------
    ...
    log_cost_lasso_coef_df = model.train_lasso(train_df,
                                               holdout_df,
                                               all_df,  ### MODIFIED BY Y. LIU TO INCLUDE ALL THE SAMPLE
                                               x_column_names,
                                               y_col='log_cost_t',
    ...
    '''
    # save to CSV
    output_filepath = os.path.join(OUTPUT_DIR, 'pred_log_cost_df.csv')
    pred_log_cost_df.to_csv(output_filepath, index=False)

    output_filepath = os.path.join(OUTPUT_DIR, 'pred_gagne_sum_t_df.csv')
    pred_gagne_sum_t_df.to_csv(output_filepath, index=False)

    output_filepath = os.path.join(OUTPUT_DIR, 'pred_log_cost_avoidable_df.csv')
    pred_log_cost_avoidable_df.to_csv(output_filepath, index=False)
    ### MODIFIED BY Y. LIU, END   ---------
